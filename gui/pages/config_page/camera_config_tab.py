...
def render_camera_config_tab():
    ...
    def on_add_camera():
        new_camera_name = new_camera_name_input.value
        if not new_camera_name:
            logger.warning("未输入摄像头节点名称")
            return
        vs = get_vision()
        vs.add_camera(new_camera_name)

    with ui.row():
        ui.button('保存配置', color='secondary', on_click=lambda e: on_save_config())
    with ui.row():
        new_camera_name_input = ui.input(label='添加摄像头节点名称', placeholder='输入摄像头节点别名')
        ui.button('添加摄像头节点', color='primary', on_click=lambda e: on_add_camera())
...
def render_camera_block(key: str, cam: Camera):
    ...
    def on_camera_change(index: int):
        ...
        logger.info(
            f'{key} 已选择摄像头 {cam.name} (index={index})')
        _refresh_header()
    ...
    def on_refresh_image():
        try:
            ...
            if frame is not None:
                ...
                logger.warning(f'摄像头 {cam.name} 图像为空图像')
    ...
    with ui.expansion(value=False).classes('w-full q-mb-md') as exp:
        ...
        with ui.card().classes('q-pa-md w-full'):
            with ui.row().classes('q-gutter-xl'):
                ...
                # 右侧：信息
                with ui.column().classes('q-gutter-sm'):
                    status_widget = ui.code(cam.get_status()).props('readonly')

        # with ui.card().classes('q-pa-md q-mb-md'):
        #     tag36h11_config = cam._tag36h11_detector.get_config(
        #     ) if cam._tag36h11_detector else None
