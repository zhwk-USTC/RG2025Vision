...
def render_localization_tab() -> None:
    vs = get_vision()
    _localization_loop = None
    with ui.row():
        def on_save_config():
            save_vision_config()
        def on_localization_loop_change(value):
            nonlocal _localization_loop
            if value:
                vs.start()
                _localization_loop = ui.timer(1.0 / debug_fps_input.value, vs.update)
                logger.info(f"已启动定位循环，频率 {debug_fps_input.value} FPS")
            else:
                if _localization_loop:
                    _localization_loop.cancel()
                    logger.info("已停止定位循环")
                    vs.stop()
                    
        ui.button('保存配置', color='secondary', on_click=on_save_config)
        ui.checkbox('测试定位循环', value=False, on_change=lambda e: on_localization_loop_change(e.value))
        debug_fps_input = ui.number('频率 (FPS)', value=5, min=1, max=30, step=1)
    return
