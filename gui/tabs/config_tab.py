...
from vision.camera import cameras, camera_info_list, save_config
...
def render_config_tab():
    ...
    def render_apriltag_config():
        ...
        with ui.card().classes('q-pa-md q-mb-md'):
            ...
            # 应用配置和保存配置按钮同一行
            from vision.apriltag import save_apriltag_config, apply_apriltag_config
            ...
            def render_cam_block(cam=cam, idx=idx):
                with ui.card().classes('q-pa-md q-mb-md'):
                    ...
                    with ui.row().classes('q-gutter-xl'):
                        ...
                        # 右侧图像与信息
                        with ui.column().classes('q-gutter-sm'):
                            img_widget = ui.image(get_placeholder_img()).style('width:320px;height:auto;object-fit:contain;display:block;').classes('shadow-2')
                    ...
                    def on_refresh_img(cam=cam, img_widget=img_widget):
                        try:
                            if cam.connected:
                    ...
                    refresh_btn.on('click', lambda e, cam=cam, img_widget=img_widget: on_refresh_img(cam, img_widget))

                    def update_info():
                        info_label.set_value(get_info_text(cam))
                    ui.timer(1, update_info)
