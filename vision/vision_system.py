...
from .camera_node import scan_cameras, CamNodeConfig
...
class VisionSystem:
    ...
    def __init__(
        ...
        self._cam_nodes: List[CameraNode] = []
        self.localizer: Optional[Localizer] = None

        scan_cameras()
        for i in range(cam_num):
            self._cam_nodes.append(CameraNode())
        logger.info(f"[VisionSystem] 已注册相机节点：{', '.join([n.alias for n in self._cam_nodes])}")
    ...
    def update(self) -> None:
        ...
        try:
            ...
            for node, res in zip(self._cam_nodes, results):
                alias = node.alias
    ...
    def set_config(self, config: VisionSystemConfig) -> None:
        ...
        try:
            for node, cam_config in zip(self._cam_nodes, cam_node_configs):
