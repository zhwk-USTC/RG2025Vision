...
class VisionSystemConfig:
    ...
    camera_metadata: Dict[str, dict] = field(
    ...
    def __init__(
        ...
        try:
            if VisionSystemConfig is not None:
                ...
                self._tag36h11_detector = Tag36h11Detector(tag36h11_detector_config)
                logger.info(f"[VisionSystem] 添加 36h11 检测器")

                self._hsv_detector = HSVDetector(VisionSystemConfig.hsv_detector)
            ...
            for k in required_keys:
                if k not in self._cameras:
                    default_cfg = CameraConfig(index=-1, width=1920, height=1080, fps=30.0)
                    self._cameras[k] = Camera(default_cfg)
                    self._camera_metadata.setdefault(k, {})  # 预留元信息字典
